...
- **10카드 라운드**: 카드 1장씩, 라운드 진행바만 보여주고 전체 마스터리는 체크포인트에서만 공개 (Brainscape의 정보 유보)
- **자기평가 3버튼**: 몰랐다(X)·부분(△)·완벽(O) + 버튼에 다음 등장 간격 미리보기 (RemNote 라벨 + Anki 간격 표시)
- **간격 엔진(고정 코드)**: X→1일, △→2일, O→연속 횟수에 따라 4·7·14·21일. X는 같은 세션 큐 끝에 재입장
- **카드 왼쪽 테두리 = 지난 판정 색** (Brainscape), **2연속 X = 고질 약점**으로 승격, 홈에서 고질 약점만 돌기 (수험가 WOX 관례)
- **이중 골**: 미니 골 10장(스트릭 유지용) + 오늘 큐 완주 (RemNote)
